...
bio_path = f"../name_bio/prompt_entities.{model_name.split('/')[-1]}.csv"
bio_pd = pd.read_csv(bio_path)
output_path = f"../name_bio/prompt_entities.{model_name.split('/')[-1]}.bio_facts_segs.json"
name2bio = {}
for i in range(len(bio_pd)):
    name = bio_pd.iloc[i]['name']
